        ...
        public virtual void Activate()
        {
            ...
            for(int i=0; i<_timestepsPerActivation; i++)
            {
                ...
                for (int j = 0; j < _connectionArray.Length; j++)
                {
                    ...
                    if (_connectionArray[j]._rwTgtFlag)
                    {
                        // Since the network is cyclic, we don't have to worry about the target connection coming before the current connection in the current layer or even if the target connection is in a layer lower than it (which is impossible I think... you know, since every neuron in a cycle is at the same depth of infinity?)
                        // Worst case scenario, the runtime weights are guaranteed to be properly set after the first timestep
                        // Set the target connection's weight to the computed runtime value
                        _connectionArray[_connectionArray[j]._tgtConnectionIdx]._weight = _postActivationArray[_connectionArray[j]._srcNeuronIdx] * _connectionArray[j]._weight;
                    }
                    else
                    {
