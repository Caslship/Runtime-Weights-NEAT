        ...
        {   
            ...
            for(int layerIdx=1; layerIdx < _layerInfoArr.Length; layerIdx++)
            {
                ...
                for(; conIdx < layerInfo._endConnectionIdx; conIdx++) 
                {
                    ...
                    // Check to see if the current connection targets a runtime weight
                    if (_connectionArr[conIdx]._rwTgtFlag)
                    {
                        // If the target connection came before the current connection, an old weight value was used instead of the current runtime weight
                        // We combat this by taking off from the target connection's target neuron's preactivation signal, changing the target connection's weight value to the correct (runtime) value, and then multiplying the runtime weight value by its source neuron's postactivation signal
                        // In acyclic networks, we know that the a runtime weight will never be located in a layer lower than itself... or else that would form a cycle which conflicts with the definition of a acyclic network
                        // Tying into what is said just above, due to the way the extension is implemented and how FastAcyclicNetwork's are set up, a connection will always target a runtime weight that is within the same layer or above 
                        // The only thing we need to be concerned about in the case of FastAcyclicNetwork is that a connection that contains a runtime weight might try to add to its target neuron's preactivation signal before its runtime weight is properly set, thus resulting in the use of an old/invalid weight value
                        if (_connectionArr[conIdx]._tgtConnectionIdx < conIdx)
                        {
                            // Remove the target connection's output (which is the postactivation signal of the source neuron times the connection's weight) from the preactivation signal of the target neuron
                            _activationArr[_connectionArr[_connectionArr[conIdx]._tgtConnectionIdx]._tgtNeuronIdx] -= _connectionArr[_connectionArr[conIdx]._tgtConnectionIdx]._srcNeuronIdx * _connectionArr[_connectionArr[conIdx]._tgtConnectionIdx]._weight;
                            // Set the target connection's weight to the computed runtime value
                            _connectionArr[_connectionArr[conIdx]._tgtConnectionIdx]._weight = _activationArr[_connectionArr[conIdx]._srcNeuronIdx] * _connectionArr[conIdx]._weight;
                            // Since it is too late to go back to the target connection and add its output value to target neuron's preactivation value, we must do so here
                            _activationArr[_connectionArr[_connectionArr[conIdx]._tgtConnectionIdx]._tgtNeuronIdx] += _activationArr[_connectionArr[_connectionArr[conIdx]._tgtConnectionIdx]._srcNeuronIdx] * _connectionArr[_connectionArr[conIdx]._tgtConnectionIdx]._weight;
                        }
                        // No need for anything special; just set the weight value within the target connection to the postactivation value of the source neuron from the current connection 
                        else
                        {
                            // Set the target connection's weight to the computed runtime value
                            _connectionArr[_connectionArr[conIdx]._tgtConnectionIdx]._weight = _activationArr[_connectionArr[conIdx]._srcNeuronIdx] * _connectionArr[conIdx]._weight;
                    ...
                    else
                    {
                        ...
                        // Add the connection's output value to the target neuron's preactivation signal
