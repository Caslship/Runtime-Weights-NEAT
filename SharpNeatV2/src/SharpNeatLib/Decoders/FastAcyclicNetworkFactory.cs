...
namespace SharpNeat.Decoders
{
    ...
    public class FastAcyclicNetworkFactory
    {
        ...
        //=== Create array of FastConnection(s). 
            ...
            // Jason Palacios - 2014 - Runtime Weight Extension - jason.palacios@utexas.edu
            // To keep track of the new connection's index in the array after sorting by using the connection's innovation ID
            Dictionary<uint, int> newConnIdxById = new Dictionary<uint, int>(connectionCount);
            for (int i = 0; i < connectionCount; i++)
            {
                newConnIdxById.Add(fastConnectionArray[i]._connectionId, i);
            }
            // Now, fix the references to a target connection so that they use an index in the sorted connection array
            for (int i = 0; i < connectionCount; i++)
            {
                if (fastConnectionArray[i]._rwTgtFlag)
                {
                    fastConnectionArray[i]._tgtConnectionIdx = newConnIdxById[fastConnectionArray[i]._tgtConnectionId];
                }
            }
