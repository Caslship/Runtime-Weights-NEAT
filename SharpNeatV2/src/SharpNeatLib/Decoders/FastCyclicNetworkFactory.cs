...
namespace SharpNeat.Decoders
{
    ...
    public class FastCyclicNetworkFactory
    {
                                           ...
                                           out double[][] neuronAuxArgsArray)
        {
            ...
            // Build dictionary of connection indexes keyed on connection innovation ID.
            int connectionCount = fastConnectionArray.Length;
            Dictionary<uint, int> connectionIndexDictionary = new Dictionary<uint, int>(connectionCount);
            for (int i = 0; i < connectionCount; i++)
            {
                connectionIndexDictionary.Add(fastConnectionArray[i]._connectionId, i);
            }
            // Fix references that target connection innovation IDs and instead have them target an index in the FastConnection array.
            for (int i = 0; i < connectionCount; i++)
            {
                if (fastConnectionArray[i]._rwTgtFlag)
                {
                    fastConnectionArray[i]._tgtConnectionIdx = connectionIndexDictionary[fastConnectionArray[i]._tgtConnectionId];
                }
            }
        ...
        private static FastConnection[] CreateFastConnectionArray(INetworkDefinition networkDef)
        {
            ...
            if((2.0 * connectionCount * Math.Log(nodeCount, 2.0)) < ((2.0 * connectionCount) + nodeCount))
            {
                ...
                {   
                    ...
                    // Jason Palacios - 2014 - Runtime Weight Extension - jason.palacios@utexas.edu
                    fastConnectionArray[i]._rwTgtFlag = conn.RuntimeWeightTargetFlag;
                    fastConnectionArray[i]._tgtConnectionIdx = -1;
                    fastConnectionArray[i]._tgtConnectionId = conn.TargetConnectionId;
                    fastConnectionArray[i]._connectionId = conn.InnovationId;
                }
            }
            // [CONTINUE] Colin Green's original code for SharpNEAT v2.0
            else
